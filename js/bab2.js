...
KKA.bab2 = {
  initHub() {
    ...
    games.forEach(function (g) {
      ...
      if (el && KKA.state && KKA.state.data && KKA.state.data.bab2 && KKA.state.data.bab2[g]) {
        el.textContent = '✅';
      }
    });
  },

  showCompletion(containerId, minigameName, badgeId, activityKey) {
    ...
    container.innerHTML = [
      ...
      '<button style="background:#4f46e5; color:white; padding:10px 20px; border:none; border-radius:8px; cursor:pointer; margin-top:10px;" onclick="KKA.ui.showScreen(\'screen-bab2-hub\')">Kembali ke Arcade</button>',
      '</div>'
    ].join('');

    if (KKA.state) {
      KKA.state.addXP(KKA.config.XP.MINIGAME_COMPLETE);
      KKA.state.completeActivity('bab2', activityKey);
      if (badgeId) {
  ...
  hint(penalty) {
    if (KKA.state) KKA.state.addXP(penalty);
    if (KKA.ui) KKA.ui.showNotification('Hint dipakai (-5 XP)', 'info');
  },
  ...
  stack: {
    ...
    push(color) {
      ...
      if (KKA.audio) KKA.audio.playClick();
      this.render();
      this.checkWin();
    },
    ...
    checkWin() {
      ...
      if (this.currentStack.length !== target.length) return;
      const win = this.currentStack.every(function (value, idx) {
      ...
      setTimeout(function () {
        ...
        if (self.levelIdx >= self.levels.length) {
          KKA.bab2.showCompletion('stack-content', 'Stack', 'stack-master', 'stack');
        } else {
          self.start();
        }
      }, 800);
    ...
    render() {
      ...
      if (!container) return;
      const target = this.levels[this.levelIdx].target;
      const colors = { merah: '#ef4444', kuning: '#f59e0b', hijau: '#10b981', biru: '#3b82f6', ungu: '#8b5cf6' };
      const targetHtml = target.map(function (c) {
        return '<div style="background:' + colors[c] + '; height:30px; border-radius:4px; text-align:center; color:white;">' + c + '</div>';
      }).join('');
      const stackHtml = this.currentStack.map(function (c) {
        return '<div style="background:' + colors[c] + '; height:30px; border-radius:4px; text-align:center; color:white; animation:slideIn 0.2s;">' + c + '</div>';
      }).join('');
      const buttons = Object.keys(colors).map(function (c) {
        return '<button style="background:' + colors[c] + '; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;" onclick="KKA.bab2.stack.push(\'' + c + '\')">Push ' + c + '</button>';
      }).join('');
      ...
      container.innerHTML = [
        ...
        '<div><h5>Target:</h5><div style="display:flex; flex-direction:column-reverse; gap:5px; border-bottom:4px solid #333; padding-bottom:5px; width:100px; height:150px; justify-content:flex-start;">' + targetHtml + '</div></div>',
        '<div><h5>Stack Kamu:</h5><div style="display:flex; flex-direction:column-reverse; gap:5px; border-bottom:4px solid #333; padding-bottom:5px; width:100px; height:150px; justify-content:flex-start;">' + stackHtml + '</div></div>',
        '</div>',
        '<div style="margin-top:20px; display:flex; gap:10px; justify-content:center; flex-wrap:wrap;">' + buttons + '<button style="background:#374151; color:white; border:none; padding:8px 16px; border-radius:4px; cursor:pointer;" onclick="KKA.bab2.stack.pop()">Pop</button><button style="background:transparent; border:1px solid #ccc; padding:8px; cursor:pointer;" onclick="KKA.bab2.hint(-5)">Hint 💡</button></div>',
        '</div>'
      ].join('');
    }
  },
  ...
  queue: {
    ...
    dequeue() {
      ...
      if (this.incoming.length === 0 && this.q.length === 0) {
        ...
        setTimeout(function () {
          self.levelIdx += 1;
          if (self.levelIdx >= self.levels.length) {
            KKA.bab2.showCompletion('queue-content', 'Queue', 'queue-pro', 'queue');
          } else {
            self.start();
          }
        }, 800);
    ...
    render() {
      ...
      if (!container) return;
      const qHtml = this.q.map(function (c) {
        return '<div style="font-size:2em; margin-right:5px; animation:scaleIn 0.2s;">' + c + '</div>';
      }).join('');
      container.innerHTML = [
        '<div style="background:white; padding:20px; border-radius:12px;">',
        ...
        '<div style="font-weight:bold; margin-right:10px;">LOKET 👈</div>' + qHtml,
        '</div>',
  ...
  array: {
    ...
    answer() {
      const input = document.getElementById('array-ans');
      if (!input) return;
      const value = parseInt(input.value, 10);
      const correct = this.qType === 0 ? value === this.arr[this.qIndex] : value === this.qIndex;
      if (correct) {
        if (KKA.audio) KKA.audio.playCorrect();
        this.questionCount += 1;
        if (this.questionCount >= this.maxQuestions) {
          KKA.bab2.showCompletion('array-content', 'Array', 'array-wizard', 'array');
        } else {
          this.generateQuestion();
        }
      } else {
        if (KKA.audio) KKA.audio.playWrong();
        if (KKA.ui) KKA.ui.showNotification('Salah, coba lagi!', 'error');
      }
    },
    ...
    render() {
      ...
      if (!container) return;
      const qText = this.qType === 0 ? 'Apa nilai di indeks ke-' + this.qIndex + '?' : 'Di indeks berapa nilai ' + this.arr[this.qIndex] + '?';
      const cells = this.arr.map(function (val) {
        return '<div style="border:2px solid #ccc; border-radius:8px; width:50px; text-align:center; overflow:hidden;">' +
      ...
      container.innerHTML = [
        ...
        '<div style="display:flex; justify-content:center; gap:5px; margin:20px 0;">' + cells + '</div>',
  ...
  linear: {
    ...
    clickBox(idx) {
      if (idx !== this.currentIndex) {
        if (KKA.audio) KKA.audio.playWrong();
        if (KKA.ui) KKA.ui.showNotification('Harus cek dari kiri ke kanan (indeks terkecil)!', 'error');
        return;
      }
      if (KKA.audio) KKA.audio.playClick();
    ...
    render() {
      ...
      if (!container) return;
      const boxes = this.arr.map(function (val, idx) {
        return '<div id="lbox-' + idx + '" onclick="KKA.bab2.linear.clickBox(' + idx + ')" style="width:60px; height:60px; border:2px solid #ccc; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:1.5em; font-weight:bold; cursor:pointer; background:#fff; color:#333;">?</div>';
      }).join('');
      container.innerHTML = [
        '<div style="background:white; padding:20px; border-radius:12px; text-align:center;">',
  ...
  binary: {
    ...
    clickMid(idx) {
      ...
      if (idx !== mid) {
        if (KKA.audio) KKA.audio.playWrong();
        if (KKA.ui) KKA.ui.showNotification('Klik nilai tengah antara indeks ' + this.left + ' dan ' + this.right + ' (yaitu ' + mid + ')!', 'error');
        return;
      }
      if (KKA.audio) KKA.audio.playClick();
    ...
    render() {
      ...
      if (!container) return;
      const cells = this.arr.map(function (val, idx) {
        const isActive = idx >= this.left && idx <= this.right;
        const isMid = isActive && idx === Math.floor((this.left + this.right) / 2);
        const style = 'width:40px; height:40px; border:2px solid ' + (isMid ? '#f59e0b' : '#ccc') + '; border-radius:4px; display:flex; align-items:center; justify-content:center; font-weight:bold; cursor:' + (isActive ? 'pointer' : 'not-allowed') + '; background:' + (isActive ? (isMid ? '#fef3c7' : '#fff') : '#e5e7eb') + '; opacity:' + (isActive ? '1' : '0.3') + ';';
        return '<div onclick="KKA.bab2.binary.clickMid(' + idx + ')" style="' + style + '">' + val + '</div>';
      }.bind(this)).join('');
  ...
  bubble: {
    ...
    action(swap) {
      ...
      if (swap === shouldSwap) {
        ...
        if (KKA.audio) KKA.audio.playWrong();
        if (KKA.ui) KKA.ui.showNotification(swap ? 'Tidak perlu ditukar!' : 'Harusnya ditukar!', 'error');
        if (shouldSwap) {
          this.arr[this.idx] = v2;
          this.arr[this.idx + 1] = v1;
        }
    ...
    render() {
      ...
      if (!container) return;
      const bars = this.arr.map(function (val, i) {
        const isHighlight = i === this.idx || i === this.idx + 1;
        const isSorted = i >= this.sortedIdx;
        const bg = isSorted ? '#10b981' : (isHighlight ? '#f59e0b' : '#6366f1');
        return '<div style="width:40px; height:' + val + 'px; background:' + bg + '; border-radius:4px 4px 0 0; display:flex; justify-content:center; align-items:flex-end; color:white; font-size:0.8em; padding-bottom:5px; transition:0.3s;">' + val + '</div>';
      }.bind(this)).join('');
  ...
  selection: {
    ...
    selectMin(idx) {
      ...
      if (idx === minIdx) {
        ...
        if (KKA.audio) KKA.audio.playWrong();
        if (KKA.ui) KKA.ui.showNotification('Salah! Nilai terkecil adalah ' + this.arr[minIdx], 'error');
      }
    },
    ...
    render() {
      ...
      const boxes = this.arr.map(function (val, idx) {
        ...
        const bg = isSorted ? '#10b981' : '#a855f7';
        const cursor = isSorted ? 'default' : 'pointer';
        return '<div onclick="KKA.bab2.selection.selectMin(' + idx + ')" style="width:60px; height:60px; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:1.5em; font-weight:bold; cursor:' + cursor + '; background:' + bg + '; color:white; transition:0.3s;">' + val + '</div>';
      }).join('');
      container.innerHTML = [
        '<div style="background:white; padding:20px; border-radius:12px; text-align:center;">',
  ...
  insertion: {
    ...
    insertAt(targetPos) {
      ...
      if (targetPos === correctPos) {
        ...
        if (KKA.audio) KKA.audio.playWrong();
        if (KKA.ui) KKA.ui.showNotification('Salah posisi! Seharusnya di posisi ' + correctPos, 'error');
      }
    ...
    render() {
      ...
      if (this.sortedCount >= this.arr.length) {
        container.innerHTML = [
          '<div style="background:white; padding:20px; border-radius:12px; text-align:center;">',
          '<h4>Selesai!</h4>',
          '<div style="display:flex; justify-content:center; gap:10px; margin:20px 0;">' + this.arr.map(function (val) {
            return '<div style="width:50px; height:70px; background:#10b981; color:white; border-radius:4px; display:flex; align-items:center; justify-content:center; font-weight:bold;">' + val + '</div>';
          }).join('') + '</div>',
          '</div>'
        ].join('');
        return;
      ...
      const remaining = this.arr.slice(this.sortedCount + 1).map(function (val) {
        return '<div style="width:40px; height:60px; background:#a855f7; color:white; border-radius:4px; display:flex; align-items:center; justify-content:center;">' + val + '</div>';
      }).join('');
      container.innerHTML = [
        '<div style="background:white; padding:20px; border-radius:12px; text-align:center;">',
